...
\begin{itemize}
\item monitors resource usage and scale up and down horizontally\endnote{I did not expect to associate up and down with horizontally and still make sense.} with the load,
...
The {\tt tee} utility gives the shell the ability to duplicate a stream. Named pipes or command substitution (for shells that support it) can be used to process data in parallel. The GNU Coreutils manual gives multiple examples, such as computing different hashes of a file while it downloads\endnote{\url{https://www.gnu.org/software/coreutils/manual/html_node/tee-invocation.html}}.
